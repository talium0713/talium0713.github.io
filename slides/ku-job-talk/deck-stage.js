...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // A hidden document delivers neither rAF nor ResizeObserver callbacks,
      // and page timers can be suspended outright — so a first fit that lands
      // on an unsettled box cannot be repaired by any in-page loop. Refit on
      // every transition into a visible or captured state instead; these
      // events do fire, and they precede the first frame anyone can see.
      this._refit = () => {
        if (this._fitting) return;
        this._fitting = true;
        try { this._fit(); } finally { this._fitting = false; }
      };
      document.addEventListener('visibilitychange', this._refit);
      window.addEventListener('pageshow', this._refit);
      window.addEventListener('load', this._refit);
      window.addEventListener('focus', this._refit);
      window.addEventListener('beforeprint', this._refit);
      // Write a scale synchronously during startup. Everything else that
      // would fit the canvas (rAF startup, ResizeObserver, window resize) is
      // either suspended or never fires in a hidden document, so this call is
      // what guarantees the deck is never left at natural size.
      this._fit();
      setTimeout(() => this._fit(), 0);
      // The host box can change without a window resize (iframe sized by its
      // parent, rail drag, panel layout). Observe the host for the page's
      // lifetime — this is what recovers a zero first measurement. _fit runs
      // synchronously behind a re-entrancy guard: it writes layout inside the
      // observed subtree, and rAF cannot be used to defer it because rAF is
      // suspended whenever the document is hidden.
      ...
      // ResizeObserver delivery (like rAF) is suspended while the document is
      // hidden, so it cannot be the guarantee — a preview iframe can be
      // resized by its host while hidden and would keep a stale scale
      // forever. This poll compares the host box against the one _fit last
      // used and is the only mechanism that always runs.
      this._fitWatch = setInterval(() => {
        if (this._fitting || !this._canvas) return;
        // Refit when the box changed OR when the canvas is not actually
        // carrying a positive scale. Keying on the box alone strands a load
        // where the first fit never wrote a transform: _fitBox memoizes the
        // current size and the poll then early-returns forever.
        const t = this._canvas.style.transform || '';
        const m = /scale\(([\d.eE+-]+)\)/.exec(t);
        const scaled = this.hasAttribute('noscale')
          ? t === 'none'
          : !!(m && parseFloat(m[1]) > 0.001);
        const r = this.getBoundingClientRect();
        const w = Math.round(r.width), h = Math.round(r.height);
        const boxSame = this._fitBox && this._fitBox.w === w && this._fitBox.h === h;
        if (scaled && boxSame) return;
        this._fitting = true;
        try { this._fit(); } finally { this._fitting = false; }
      }, 250);
      ...
      // A hidden document never delivers rAF, so a timer runs the same body
      // once as a fallback — otherwise startup never completes offscreen.
      let started = false;
      const startup = () => {
        if (started) return;
        started = true;
    ...
    disconnectedCallback() {
      if (this._refit) {
        document.removeEventListener('visibilitychange', this._refit);
        window.removeEventListener('pageshow', this._refit);
        window.removeEventListener('load', this._refit);
        window.removeEventListener('focus', this._refit);
        window.removeEventListener('beforeprint', this._refit);
      }
      clearTimeout(this._fitRetry);
      clearInterval(this._fitWatch);
    ...
    _fit() {
      if (!this._canvas) {
        // The canvas is built later in startup, so an early _fit (the
        // synchronous one in connectedCallback) has nothing to scale yet.
        // Retry on a timer instead of bailing: rAF and ResizeObserver are
        // both suspended in a hidden document, and a host that never fires a
        // window resize would otherwise leave the deck at natural size.
        if (this.isConnected && (this._fitTries = (this._fitTries || 0) + 1) < 300) {
          clearTimeout(this._fitRetry);
          this._fitRetry = setTimeout(() => this._fit(), 16);
        }
        return;
      }
      ...
      // Prefer the host's own box: an iframe sized by its host can report
      // window.inner* as 0 on the first fit, which would lock the canvas at
      // scale(0) forever (no window resize event ever follows). Fall back to
      // the viewport only when the host box has no layout yet.
      const hostBox = this.getBoundingClientRect();
      const baseW = hostBox.width || window.innerWidth;
      const baseH = hostBox.height || window.innerHeight;
      const vw = baseW - rw;
      const vh = baseH;
      const s = Math.min(vw / this.designWidth, vh / this.designHeight);
      if (!(s > 0)) {
        // Box hasn't settled (host still narrower than the rail). Keep the
        // last known-good scale rather than writing a sentinel, and retry on
        // a timer — rAF is suspended while the document is hidden, so it
        // cannot be used for recovery here.
        if (this._lastScale > 0) this._canvas.style.transform = `scale(${this._lastScale})`;
        clearTimeout(this._fitRetry);
        this._fitRetry = setTimeout(() => this._fit(), 32);
        return;
      }
      clearTimeout(this._fitRetry);
      this._fitTries = 0;
      this._lastScale = s;
      this._fitBox = { w: Math.round(hostBox.width), h: Math.round(hostBox.height) };
